...
obs_err_16basins <- bind_rows(lapply(unique(flux_data$Abbr), function(site) {
  mutate(get_obs_errs(filter(flux_data, Abbr==site)), Site=site)
})) %>% select(Site, everything())
write.csv(obs_err_16basins, file='obs_err_16basins.csv', row.names=FALSE)
...
obs_err_16basins %>% 
  ...
  ggplot(aes(x=Site, y=Var, color=Cor_Ratio, shape=Cor_TN)) + geom_point(alpha=0.5) + facet_grid(Solute ~ ., scales='free_y') +
  theme_bw() + theme(axis.text.x = element_text(angle=90))
ggsave(filename='obs_err_16basins.png', height=8, width=8)
